 ...
 */
function pluginInitPy(profile: ResolvedProfile['profile'], skills: { name: string; description: string }[]): string {
  const slug = slugify(profile.name);
  const lines = [
...
function planMcpMerge(
  ...
  return { op: { kind: 'write-file', content: dumped, destinationPath: cfgPath }, hasDirect, patEnvKey };
}
...
export const hermesAdapter: TargetAdapter = createTargetAdapter({
  ...
  planOperations(resolved: ResolvedProfile, input: ResolveInput, adapter: TargetAdapter): InstallPlan {
    ...
    if (ruleArtifacts.length > 0) {
      ...
      const body = ruleArtifacts
        .map((a) => {
          ...
          return src.type === 'inline' ? src.content : `# ${a.resource.name}\n(non-inline source skipped)`;
        })
        .filter(Boolean)
        .join('\n\n---\n\n');
      if (body) {
...
let lastPlanWarnings: HermesPlanWarnings = { skipped: [], patEnvKey: '', pluginSlug: '', needsPluginEnable: false };
