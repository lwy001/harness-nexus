...
- Design W8: [`design/phase-9-w8-sender.md`](./design/phase-9-w8-sender.md) — **designed, not implemented**: the chat Sender (composer) upgrade — a card-style two-row input (autogrow textarea, context-usage meter, circular send/stop toggle) shaped after the reference composer but styled per Signal, with zero wire changes; attach / permission-mode / model / effort controls enumerated as deferred stubs with the wire arms each needs. Also records the claude-acp-bridge reference-code adoption notes.
