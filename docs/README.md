...
- Design W11: [`design/phase-9-w11-adapter-lifecycle.md`](./design/phase-9-w11-adapter-lifecycle.md) — **B shipped** (channel snapshot + tab bar + 一键清理; server+web): `chat:channels` per-user snapshot pushes with `channels.sync` mount catch-up, the live-channel tab strip on both chat pages (tab switch keeps channels alive; page exit no longer auto-closes), and `chat:channels.closeAll` (idle now, mid-turn deferred). Still designed-only: the daemon pgid ledger + boot sweep, the adapter-process report route/panel, a listing TTL cache, and the `/ctl` disconnect grace window.
