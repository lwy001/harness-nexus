...
- Design W11: [`design/phase-9-w11-adapter-lifecycle.md`](./design/phase-9-w11-adapter-lifecycle.md) — **designed**: adapter process lifecycle & session truth — the daemon-side adapter pgid ledger with boot sweep (kills the orphaned-adapter class), `chat:channels` snapshot pushes so the session rail's 已打开 state is live, an on-demand adapter-process report (`GET /api/machines/:id/adapters` + MachineDetail panel), a listing TTL cache, and a `/ctl` disconnect grace window.
