...
- Design W7.1: [`design/phase-9-w7.1-dsh-event-tap.md`](./design/phase-9-w7.1-dsh-event-tap.md) — **designed, not implemented**: the in-process dsh cordis-plugin event tap (spawn-time `--patch` insert of a zero-dep plugin streaming the `session/event` bus back to the daemon over localhost JSON-lines), replacing the file tail as primary streaming source with the tail kept as fallback; includes the rig spike results that verified the mount mechanism.
